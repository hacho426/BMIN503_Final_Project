...
Based on NHATS/NOSC researchers, there was published algorithm that I can utilized to choose probable and possible dementia participants.
...
# Preparing dataset - recoding some covariates without labels
dementia1 <- dementia1 |> 
  mutate(
    race_recode = case_when(
      crl11dcgracehisp == 1 ~ "1",
      crl11dcgracehisp == 2 ~ "2",
      crl11dcgracehisp == 3 ~ "3",
      crl11dcgracehisp == 4 ~ "4",
      TRUE ~ NA_character_
    ),
    gender_recode = case_when(
      as.character(c11gender) == "1" ~ "1",
      as.character(c11gender) == "2" ~ "2",
      TRUE ~ NA_character_
    ),
    edu_recode = case_when(
      chd11educ %in% c(1, 2, 3) ~ "1",
      chd11educ %in% c(4, 5) ~ "2",
      chd11educ %in% c(6, 7, 8) ~ "3",
      chd11educ == 9 ~ "4",
      TRUE ~ NA_character_
    ),
    martstat_recode = case_when(
      chd11martstat == 1 ~ "1",
      chd11martstat %in% 2:6 ~ "2",
      TRUE ~ NA_character_
    )
  )
```
...
Based on a thorough examination of the literature and clinical experiences, I have identified these caregiving activities as the initial key factors to explore for predicting the mental health of caregivers. This selection is grounded in evidence from existing research and practical insights, aiming to provide a comprehensive understanding of the elements that most significantly impact caregiver well-being.
...
dementia1 <- dementia1 |>
  ...
  mutate(
    cca11hwoftchs_recoded = case_when( #CA1 HOW OFTEN HELP WITH CHORES
      cca11hwoftchs %in% c(1, 2, 3) ~ 1,
      cca11hwoftchs %in% c(4, 5) ~ 0,
      TRUE ~ NA_real_
    ),
    cca11hwoftshp_recoded = case_when( #HOW OFTEN SHOPPED FOR SP
      cca11hwoftshp %in% c(1, 2, 3) ~ 1,
      cca11hwoftshp %in% c(4, 5) ~ 0,
      TRUE ~ NA_real_
    ),
    cca11hwoftpc_recoded = case_when( #HOW OFTEN HELP PERS CARE
      cca11hwoftpc %in% c(1, 2, 3) ~ 1,
      cca11hwoftpc %in% c(4, 5) ~ 0,
      TRUE ~ NA_real_
    ),
    cca11hwofthom_recoded = case_when( #HOW OFT HLP GETTING ARDUND HOME
      cca11hwofthom %in% c(1, 2, 3) ~ 1,
      cca11hwofthom %in% c(4, 5) ~ 0,
      TRUE ~ NA_real_
    ),
    cca11hwoftdrv_recoded = case_when( #HOW OFTEN DROVE SP 
      cca11hwoftdrv %in% c(1, 2, 3) ~ 1,
      cca11hwoftdrv %in% c(4, 5) ~ 0,
      TRUE ~ NA_real_
    ),
    cca11hwoftott_recoded = case_when( # HOW OFTN WENT ON OTH TRANSPRORT
...
#C11 CA3 EVER HELP ORDER MEDICINES (cca11hlpordmd) - binary questions: yes --> 1 , recoded no (2)-->0
...
#che11enrgylmt #Energy often limited 
#cac11diffphy # Caregiver physical difficulty helping 
#cac11exhaustd #Caregiver exhausted at night 
#cac11toomuch #Care more than can handle 
#cac11uroutchg #Care routine then changes 
#cac11notime #No time for self (-8,-7,-6) 
#cac11diffemlv #Caregiver emotional difficulty (-1, 1-5)
#cpp11hlpkptgo #Kept from going out (-6-1, 1,2) 
#cac11diffinc #Caregiver financial difficulties (-8,-7,-6, binary 1,2) 
#cpp11hlpkptgo #Kept from going out (-6-1, 1,2) 

dementia1 <- dementia1 |>
  mutate(across(
    c(che11enrgylmt, cac11diffphy, cac11exhaustd, 
      cac11toomuch, cac11uroutchg, cac11notime,cac11diffemlv, cpp11hlpkptgo, cac11diffinc, cpp11hlpkptgo),
    ~ case_when(
        . == 1 ~ 1,
        . == 2 ~ 0,
        . %in% c(-8, -6, -1) ~ NA_real_,
...
# Creating subset to see Pearson's coefficient
selected_vars1 <- c("spid", "demclas", "cac11diffphy", "cac11exhaustd", "cac11diffemlv", "cca11hwoftchs", "cca11hwoftshp", "cca11hwoftpc",
  "cca11hwofthom", "cca11hwoftdrv", "cca11hwoftott",
  "che11enrgylmt", "cpp11hlpkptgo", "che11health", 
  "che11sleepint", "ew11progneed1", "ew11finhlpfam", 
  "mc11havregdoc", "hc11hosptstay", "hc11hosovrnht", 
  "cac11diffinc", "race_recode", "gender_recode", 
  "edu_recode", "chd11dage", "martstat_recode", 
  "phq2_cg_cat", "gad2_cg_cat"
)
dementia_subset1 <- dementia1 |> select(all_of(selected_vars1))

# Select only numeric columns
numeric_dementia2 <- dementia_subset1 |> select(where(is.numeric))

# Compute the Pearson correlation matrix and p-values
cor_matrix1 <- rcorr(as.matrix(numeric_dementia2), type = "pearson")

# Extract correlation coefficients and p-values
cor_coefficients <- cor_matrix1$r
p_values <- cor_matrix1$P
...
Pearson's correlation matrices were presented to visually assess the data and evaluate the independence of variables. The Pearson's correlation coefficient quantifies the linear relationship between two continuous variables, with values ranging from −1 to +1. A coefficient of 0 indicates no linear correlation, while negative and positive values represent negative and positive correlations, respectively. A p-value of \< .05 was used to define statistical significance. Then I selected the most representative variable to reduce redundancy in concepts among highly correlated items. The negative values in the dataset were not addressed in this step. Based on the Pearson's coefficient, I decided that GAD-2 (Anxiety) and PHQ-2 (Depression) could have similar variables as they appear to be correlated.
...
dementia_subset1[dementia_subset1 < 0] <- NA

#imputation
continuous_vars <- sapply(dementia_subset1, is.numeric)
dementia_subset1[continuous_vars] <- lapply(dementia_subset1[continuous_vars], function(x) ifelse(is.na(x), median(x, na.rm = TRUE), x))

# categorical: change NA to max
categorical_vars <- sapply(dementia_subset1, is.character)
dementia_subset1[categorical_vars] <- lapply(dementia_subset1[categorical_vars], function(x) {
  x <- as.factor(x)
...
remaining_nas <- sapply(dementia_subset1, function(x) sum(is.na(x)))
print(remaining_nas)
```

```{r}
#One more step: cleaning the data to select one caregiver per participant. 
final_dementia <- dementia_subset1 |>
  group_by(spid) |>
  slice_head(n = 1) |>
  ungroup()
...
remaining_nas <- sapply(depression, function(x) sum(is.na(x)))
print(remaining_nas)
```
...
For the variable "Race," in the first group 'Probable Dementia' (N = 331), 46% (98 individuals) were White, non-Hispanic, 39% (82 individuals) were Black, non-Hispanic, 3.3% (7 individuals) were of another race, and 11% (24 individuals) were Hispanic. There were 120 missing values. In the second group 'Possible Dementia' (N = 232), 61% (54 individuals) were White, non-Hispanic, 31% (28 individuals) were Black, non-Hispanic, 2.2% (2 individuals) were of another race, and 5.6% (5 individuals) were Hispanic. There were 143 missing values. The p-value for this variable is 0.13.

For the variable "Gender," in the first group, 29% (64 individuals) were male and 71% (155 individuals) were female, with 112 missing values. In the second group, 34% (32 individuals) were male and 66% (61 individuals) were female, with 139 missing values. The p-value for this variable is 0.4.

For the variable "Education," in the first group, 42% (89 individuals) had an education below high school, 45% (96 individuals) had some college education, and 13% (27 individuals) had a college education or beyond. There were 119 missing values. In the second group, 49% (44 individuals) had an education below high school, 41% (37 individuals) had some college education, and 10% (9 individuals) had a college education or beyond. There were 142 missing values. The p-value for this variable is 0.5.

For the variable "Marital Status," in the first group, 47% (74 individuals) were not married and 53% (83 individuals) were married, with 174 missing values. In the second group, 41% (29 individuals) were not married and 59% (41 individuals) were married, with 162 missing values. The p-value for this variable is 0.4. For the variable "Age," the mean age in the first group was 63 years with a standard deviation of 19 years, and there were 112 missing values. In the second group, the mean age was 62 years with a standard deviation of 21 years, and there were 139 missing values. The p-value for this variable is 0.8. The statistical tests used include Fisher’s exact test, Pearson’s Chi-squared test, and the Wilcoxon rank sum test.
...
final_dementia <- final_dementia |> 
  mutate(
    ...
    gender_recode = case_when(
      as.character(gender_recode) == "1" ~ "Male",
      as.character(gender_recode) == "2" ~ "Female",
      TRUE ~ NA_character_
    ),
    edu_recode = case_when(
...
I calculated the odds ratios and confidence intervals for the `gad2_cg_cat` and `phq2_cg_cat` models. Then, I combined these results into tidy tables for each model and printed them. Additionally, I used the `broom` package to create tidy results for both models, including exponentiated coefficients and confidence intervals, and printed these as well.
...
I performed a 10-fold cross-validation on a logistic regression model to predict depression (phq2). The workflow was created, and the model was fitted using the cross-validation folds, achieving an AUC of 0.8314. Predictions were collected and ROC curves were plotted to visualize the model's performance. Next, I made predictions on the test data and plotted the ROC curve for these predictions. The model's AUC on the test data was 0.8674, indicating good predictive performance. Metrics for the test data predictions were also collected for further analysis.
...
For model validation, I performed 10-fold cross-validation, integrating the random forest model into a workflow. The model achieved a strong **ROC-AUC score of 0.91**, demonstrating excellent classification performance.
...
anxiety_rf_pred_class <- predict(anxiety_rf_fit, new_data = anxiety_test)
str(anxiety_rf_pred_class) 
...
anxiety_rf_pred_values <- bind_cols(
  truth = anxiety_test$gad2_cg_cat,  # 실제 클래스 값
  predicted_class = anxiety_rf_fit$.pred_class,  # 예측된 클래스
  predicted_prob_0 = anxiety_rf_fit$.pred_0,  # 클래스 0에 대한 예측 확률
  predicted_prob_1 = anxiety_rf_fit$.pred_1,  # 클래스 1에 대한 예측 확률
)


anxiety_rf.pred.values <- bind_cols(
  truth = anxiety_test$gad2_cg_cat,
  predict(anxiety_rf_fit, ),
  predict(anxiety_rf_fit, , type = "prob")
)
head(anxiety_rf_fit)
...
2.  **che11energylmt**: Caregivers' feeling of lacking energy.

3.  **cac11diffphy**: Caregivers' difficulty with providing physical help.

4.  **che11sleepint**: Caregivers' difficulty with sleep.

5.  **che11health**: Caregivers' own health problems.
...
collect_metrics(rf_wf_fit_cv_anxiety) #roc_auc = 0.9012371		
...
I performed a 10-fold cross-validation on a random forest model to predict anxiety (gad2_cg_cat). The workflow was created, and the model was fitted using the cross-validation folds. The initial AUC was 0.8978, and after saving predictions, the AUC slightly improved to 0.9012. ROC curves were plotted to visualize the model's performance, and metrics were collected for further analysis.

### RF- Depression

I created a random forest model with 1000 trees and a minimum node size of 5 to classify depression using the `phq2_cg_cat` variable. After fitting the model to the training data, I extracted the importance of the top variables, using the `MeanDecreaseGini` measure to identify the most influential features. Different from anxiety, new variable chd11dage (caregiver's age) is one of the influential feature.
...
The analysis revealed that, unlike the anxiety model, a new variable, `chd11dage` (caregiver's age), emerged as one of the top influential features. This indicates that the age of the caregiver plays a significant role in predicting depression, highlighting a key difference in the factors contributing to depression compared to anxiety. This insight can be crucial for tailoring interventions and support for caregivers based on their age.
...
collect_metrics(rf_wf_fit_cv_depression) #roc_auc = 0.8814059		
...
I created a random forest model specification (`rf_spec_depression`) with 1000 trees and a minimum node size of 5, setting it to use the "randomForest" engine with importance measures enabled. This model was then fitted to the depression training data (`phq2_cg_cat`). Next, I performed 10-fold cross-validation on this random forest model using the training data. The workflow was created and fitted to the cross-validation folds, achieving an initial AUC of 0.8841. After saving predictions, the AUC was slightly adjusted to 0.8814. I also plotted the ROC curve to visualize the model's performance and collected metrics from the resampling results to evaluate the model's effectiveness.

## **Comparison of ROC Curves for Logistic Regression and Random Forest Models in Anxiety and Depression Prediction**

I generated ROC curves for both the logistic regression and random forest models on the training and cross-validation sets for anxiety and depression prediction. The ROC curves were plotted using `ggplot2` to compare the performance of the models. The plot includes: **Logistic Regression (Training)**: Solid blue line, **Logistic Regression (10-fold CV)**: Dashed green line, **Random Forest (Training)**: Solid red line, and **Random Forest (10-fold CV)**: Dashed purple line. This visualization helps in comparing the true positive rate (sensitivity) against the false positive rate (1 - specificity) for each model and dataset, providing insights into their predictive performance.
...
The implications of this study highlight the significant factors contributing to anxiety and depression among caregivers. By identifying key predictors such as caregivers' feelings of exhaustion, lack of energy, difficulty with physical help, sleep issues, and health problems, the study highlighted the multifaceted challenges faced by caregivers. Additionally, the discovery that caregiver's age is a significant predictor of depression provides new insights for targeted interventions.

Given that this was an exploratory study, the findings serve as a foundation for future research. Future studies should aim to validate these results with larger and more diverse samples, and explore the underlying mechanisms driving these associations. Longitudinal studies are particularly valuable as they can track changes over time, offering insights into how these factors evolve and their long-term impact on caregivers. Since the NHATS (National Health and Aging Trends Study) and NSOC (National Study of Caregiving) are longitudinal studies, obtaining the next round of data from these sources will be crucial. This will allow for a more comprehensive analysis and help in identifying trends and patterns that can inform targeted interventions and support programs for caregivers.

Overall, this exploratory research paves the way for more comprehensive investigations and the development of tailored support programs to improve the well-being of caregivers.
...
This exploratory study highlights the critical factors influencing anxiety and depression among caregivers, providing a foundation for future research and targeted interventions. Further validation and longitudinal studies are needed to deepen our understanding and support the well-being of caregivers.
