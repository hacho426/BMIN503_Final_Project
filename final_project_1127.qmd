...
dementi1_clean <- dementi1_clean |>
  mutate(strain = rowSums(select(.data, ends_with("_recode")), na.rm = TRUE))
...
selected_cols <- select(dementi1_clean, ends_with("_recode"))
head(selected_cols)
```
...
*A total of The mean age of the caregivers was 62.6 ±13.5 (20–95) years. The majority of participants were female (65.9%), high school graduates (52.3%), married or living with a partner (57.8%), and employed (64.4%). Approximately 72% of the care recipients were identified as having probable dementia, whereas 28% were classified as having possible dementia.*
