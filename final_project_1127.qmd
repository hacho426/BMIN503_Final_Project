...
collect_metrics(rf_fit_cv) #roc_auc= 0.4855333	
...
rf_pred <- predict(rf_model, newdata=depression2)
conf_matrix_rf <-confusionMatrix(rf_pred, depression2$phq2_cg_cat)
precision_rf <- conf_matrix_rf$byClass["Pos Pred Value"]  # Precision #0.984
recall_rf <- conf_matrix_rf$byClass["Sensitivity"]        # Recall #1
f1_score_rf <- 2 * (precision_rf * recall_rf) / (precision_rf + recall_rf) 
cat("F1-score_svm:", f1_score_rf)  # F1-score = 0.9919137

#### Set up the trainControl for 5-fold cross-validation
train_control <- trainControl(method = "cv", number = 5)
# Step 1: Predict probabilities
rf_probs <- predict(rf_model, newdata = depression2, type = "prob")

# Compute ROC Curve
roc_obj <- roc(depression2$phq2_cg_cat, rf_probs[, "true"])  
